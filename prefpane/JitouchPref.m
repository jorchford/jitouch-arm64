...
- (NSString *)generateJitouchLaunchAgent {
    ...
    NSString *programPath = [NSString stringWithFormat:@"%@/Contents/Resources/Jitouch.app/Contents/MacOS/Jitouch", pathToUs];
    NSString *appResourceMarker = @"/Contents/Resources/Jitouch.prefPane";
    NSRange embeddedRange = [pathToUs rangeOfString:appResourceMarker options:NSBackwardsSearch];
    if (embeddedRange.location != NSNotFound) {
        NSString *outerAppPath = [pathToUs substringToIndex:embeddedRange.location];
        programPath = [NSString stringWithFormat:@"%@/Contents/MacOS/Jitouch", outerAppPath];
    }
...
"</dict>\n"
"</plist>";
    NSString *launchAgent = [NSString stringWithFormat:launchAgentFmt, programPath, home];
    return launchAgent;
}
