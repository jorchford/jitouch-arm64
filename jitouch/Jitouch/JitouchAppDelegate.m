...
- (NSString *)generateJitouchLaunchAgent {
    NSString *pathToExecutable = [[[NSBundle mainBundle] executablePath] stringByStandardizingPath];
...
"</dict>\n"
"</plist>";
    return [NSString stringWithFormat:launchAgentFmt, pathToExecutable, home];
}

- (BOOL)isRunningFromApplicationsFolder {
    NSString *bundlePath = [[[NSBundle mainBundle] bundlePath] stringByStandardizingPath];
    NSString *userApplications = [[NSHomeDirectory() stringByAppendingPathComponent:@"Applications"] stringByAppendingString:@"/"];
    return [bundlePath hasPrefix:@"/Applications/"] || [bundlePath hasPrefix:userApplications];
}

- (void)ensureJitouchLaunchAgent {
    ...
    NSString *plistPath = [@"~/Library/LaunchAgents/com.jitouch.Jitouch.plist" stringByStandardizingPath];
    NSError *error = nil;
    NSString *existing = [NSString stringWithContentsOfFile:plistPath encoding:NSUTF8StringEncoding error:&error];
    if (existing && [existing isEqualToString:launchAgent])
        return;

    NSString *launchAgentPath = [@"~/Library/LaunchAgents" stringByStandardizingPath];
    NSFileManager *fm = [NSFileManager defaultManager];
    BOOL isDir = NO;
    if (![fm fileExistsAtPath:launchAgentPath isDirectory:&isDir]) {
        [fm createDirectoryAtPath:launchAgentPath withIntermediateDirectories:YES attributes:nil error:&error];
    }
